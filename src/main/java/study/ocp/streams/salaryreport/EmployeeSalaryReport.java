package study.ocp.streams.salaryreport;
...
public class EmployeeSalaryReport{
	public static void main(String... args){
		...
		System.out.println(employees.stream().collect(Collectors.groupingBy(Emp::dept,Collectors.mapping(Emp::name,Collectors.toSet()))));

		// group by dept, sum of salary
		System.out.println(employees.stream().collect(Collectors.groupingBy(Emp::dept,Collectors.summingDouble(Emp::salary))));
	...
	static Map<String, Double> avgSalaryByDept(List<Emp> emps){
		return emps.stream().collect(Collectors.groupingBy(Emp::dept,Collectors.averagingDouble(Emp::salary)));
	}
