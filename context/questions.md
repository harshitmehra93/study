...
| 6 | DP | ✅ |  | Partition Equal Subset Sum |  |
| 7 | DP | ✅ |  | Target Sum |  |
...
| 48 | Trees | ✅ |  | Diameter of Binary Tree |  |
...
| 65 | Heap / Priority Queue | 🟡✅(redo done) |  | Find Median from Data Stream — guided, reattempt later |  |
...
| 76 | Sliding Window | 🟡 pattern understood, redo once later. ✅(redo done)✅ |  | Minimum Window Substring |  |
...
| 88 | Intervals | ✅ brute force understood; 🟡 optimal heap sweep not owned; ✅ optimal heap sweep implemented; 🟡 redo once later for ownership | L2 review 2026-06-24 | MinimumIntervalToIncludeEachQuery | Recall attempt identified sorting intervals and using a size-ordered heap, but the full sweep was not independently derived. Key missing pieces: sort queries with original indices, add every interval once when `left <= query`, and lazily remove expired heap-top intervals when `right < query`. Redo later from a blank method. |
...
| 92 | Greedy | ✅ DP/BFS-style solution done; ✅ greedy range-boundary solution implemented; 🟡 not installed |  | Jump Game II | Track the largest possible index a current step can reach. Do a pass from 0 to n-1; when index reaches current boundary, increment step and update the largest possible boundary. |
| 93 | Greedy | ✅ brute force; 🟡 greedy pattern not installed |  | Gas Station |  |
...
| 127 | Tries | 🟡 |  | Word Search II | Correct high-level Trie + board DFS model attempted. Key state lesson: in DFS, `node` represented the trie state before consuming the current board char, so word detection belongs on `nextNode = node.nodes[c]`. Remaining correctness issue: visited cells must be marked/unmarked on the current path. |
